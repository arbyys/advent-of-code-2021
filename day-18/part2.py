...
for indexLoop in range(0, len(data)):
    print(indexLoop+1, "/", len(data))
    currentLoopNumber = data[indexLoop]
    for index in range(0, len(data)):
        if(index == indexLoop):
            continue
        currentNumber = add(currentLoopNumber, data[index])
        while True:
            ...
            if(isSplittable(currentNumber)):
                ...
                if(isExplodable(currentNumber)):
                    continue
                break
        currentMagnitude = magnitude(currentNumber)
